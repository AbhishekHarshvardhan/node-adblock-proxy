	...
	var _parse_file = function(url) {
		...
		_fs.exists(path, function(exists) {
			...
			if (exists === true) {
				...
				// 1. JSON config
				var tmp = path.split('/'); tmp.pop(); tmp = tmp.pop();
...
//console.log(line);
	...
	var Proxy = function(data) {
		...
		console.log("🚀 ~ file: proxy.js ~ line 386 ~ Proxy ~ this.root", this.root);
		this.port      = settings.port;
		console.log("🚀 ~ file: proxy.js ~ line 388 ~ Proxy ~ this.port", this.port);
		this.ispublic  = settings.ispublic;
		console.log("🚀 ~ file: proxy.js ~ line 390 ~ Proxy ~ this.ispublic", this.ispublic);
		...
		listen: function(type, port) {
			...
			if (
				...
				if (_server !== undefined) {
					...
					console.log("🚀 ~ file: proxy.js ~ line 426 ~ host", host);


					_server.create(port, host, _check_request, this);
					console.log(type.toUpperCase() + ' Proxy created on port ' + port + ' host ' + host);
